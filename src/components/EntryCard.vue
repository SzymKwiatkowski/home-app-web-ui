<template>
  <div class="entry-card card-flat" :class="entry.type">
    <div class="entry-left">
      <div class="entry-icon" :style="iconStyle">
        {{ entryIcon }}
      </div>
      <div class="entry-info">
        ...
        <div class="entry-meta text-xs text-muted">
          ...
          <span class="dot">·</span>
          <span :class="['badge', entry.type === 'expense' ? 'badge-expense' : 'badge-event']">
            {{ entry.type === 'expense' ? typeName : 'Event' }}
          </span>
        </div>
        <div v-if="entry.description" class="entry-desc text-sm text-secondary">
          {{ entry.description }}
        </div>
      </div>
    </div>
    <div class="entry-right">
      <div v-if="entry.type === 'expense'" class="entry-amount">
...
const typeName = computed(() => typeObj.value?.name || 'Uncategorized')
...
const iconStyle = computed(() => ({
  background: props.entry.type === 'event' ? 'var(--color-accent-light)' : (typeObj.value?.color + '18' || 'var(--color-surface-2)'),
}))
...
.entry-amount { font-weight: 600; font-size: 1rem; color: var(--color-expense); }
